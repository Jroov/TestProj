...
@ SpellSystem.c:13: 	SpellList* ROMList = SpellListTable[unit->pCharacterData->number];
	...
	ldr	r7, .L69+4	@ <retval>,
	ldr	r4, [r3, r2]	@ ROMList, SpellListTable
...
@ SpellSystem.c:167: 		buffer->attributes |= BATTLE_HIT_ATTR_HPSTEAL; // "HP drain" bit.
	movs	r2, #128	@ tmp141,
	ldr	r3, [r4]	@ tmp156,* buffer
	lsls	r2, r2, #1	@ tmp141, tmp141,
	orrs	r2, r3	@ tmp139, tmp156
@ SpellSystem.c:169: 		buffer->damage -= cost;
	ldrb	r3, [r4, #5]	@ tmp143,
	subs	r3, r3, r0	@ tmp145, tmp143, tmp153
@ SpellSystem.c:167: 		buffer->attributes |= BATTLE_HIT_ATTR_HPSTEAL; // "HP drain" bit.
	str	r2, [r4]	@ tmp139,* buffer
@ SpellSystem.c:169: 		buffer->damage -= cost;
...
@ SpellSystem.c:174: 		buffer->attributes |= BATTLE_HIT_ATTR_5; // This bit is checked in an external hack I've made.
	movs	r3, #32	@ tmp149,
	ldr	r2, [r4]	@ tmp157,* buffer
	orrs	r3, r2	@ tmp147, tmp157
...
.L233:
	ldr	r3, [sp]	@ ivtmp.339, %sfp
	adds	r3, r3, #1	@ ivtmp.339,
	str	r3, [sp]	@ ivtmp.339, %sfp
	b	.L231		@
.L232:
@ RangeDisplay.c:52: 			if ( usability(unit,spell) ) { current = IncorporateNewRange(current,gGet_Item_Range(unit,spell)); }
	ldr	r3, [sp, #4]	@ usability, %sfp
...
@ RangeDisplay.c:10: 	long long current = 0;
	movs	r5, #0	@ <retval>,
	movs	r3, r0	@ _19, unit
	movs	r7, r0	@ ivtmp.348, unit
	movs	r4, r5	@ <retval>, <retval>
	adds	r3, r3, #40	@ _19,
	str	r3, [sp, #12]	@ _19, %sfp
	adds	r7, r7, #30	@ ivtmp.348,
.L249:
...
@ RangeDisplay.c:14: 		for ( int i = 0 ; i < 5 && unit->items[i] ; i++ )
	...
	adds	r7, r7, #2	@ ivtmp.348,
	cmp	r7, r3	@ ivtmp.348, _19
	bne	.L249		@,
.L251:
...
.L305:
	...
	adds	r4, r4, r3	@ ivtmp.382, tmp300, tmp434
	ldr	r3, [sp, #8]	@ _173, %sfp
	ldr	r6, [sp, #8]	@ ivtmp.380, %sfp
	adds	r3, r3, #76	@ _173,
	str	r3, [sp, #16]	@ _173, %sfp
	adds	r6, r6, #52	@ ivtmp.380,
.L303:
@ SpellMenu.c:115: 	for ( int i = 0 ; i < 3 ; i++ ) { Text_Display(&menuItemPanel->textHandles[i],&gBG0MapBuffer[y+1+2*i][x+1]); }
	movs	r1, r4	@, ivtmp.382
	movs	r0, r6	@, ivtmp.380
	ldr	r3, .L315+80	@ tmp330,
	bl	.L14		@
@ SpellMenu.c:115: 	for ( int i = 0 ; i < 3 ; i++ ) { Text_Display(&menuItemPanel->textHandles[i],&gBG0MapBuffer[y+1+2*i][x+1]); }
	ldr	r3, [sp, #16]	@ _173, %sfp
	adds	r6, r6, #8	@ ivtmp.380,
	adds	r4, r4, #128	@ ivtmp.382,
	cmp	r6, r3	@ ivtmp.380, _173
	bne	.L303		@,
