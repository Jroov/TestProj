...
int GetBattleUnitExpGain(BattleUnit* actor, BattleUnit* target){
	...
	if (CanBattleUnitGainExp(actor, target)){
		...
		int divisor = 1; //divide by 1 (keep same) if not fatigued, otherwise by 2
		...
		if (target->unit.curHP == 0){
			...
			int killexp;
			int initialKillExp = (28*bossFactor + (4 * GetLevelDifference(actor, target)))/divisor;
			if(initialKillExp <= 0){
				killexp = (5/divisor);
			}
				...
				killexp = initialKillExp;
			}
			return killexp;
		}

		// hit
		int initialHitExp = (10 + (2 * GetLevelDifference(actor, target)))/divisor;
			if(initialHitExp <= 0){
				return (2/divisor);
			}
